...
const CartPage = () => {
    ...
    return (
        <div className="bg-clinical-canvas min-h-screen pt-32 pb-40">
            <Helmet>
                <title>Your Cart | Olive Edge</title>
                        ...
                        <div className="flex-1 space-y-12">
                            {cartItems.map((item) => (
                                <div key={item.product} className="flex flex-col md:flex-row items-center gap-8 group">
                                    ...
                                    {/* Image */}
                                    <Link to={`/product/${item.product}`} className="w-32 h-40 bg-clinical-bone flex-shrink-0 overflow-hidden relative rounded-3xl">
                                        <img src={item.image} alt={item.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                                    </Link>

                                    {/* Info */}
                                    <div className="flex-1 text-center md:text-left">
                                        <Link to={`/product/${item.product}`} className="block font-display text-2xl text-clinical-ink mb-2">
                                            {item.name}
                                        </Link>
