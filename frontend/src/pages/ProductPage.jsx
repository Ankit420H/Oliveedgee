...
import ProductCard from '../features/product/components/ProductCard'; // Import ProductCard
...
const ProductPage = () => {
    ...
    // const { user } = useContext(AuthContext); // Removed unused
    const [product, setProduct] = useState(null);
    const [qty, setQty] = useState(1);
    // Removed unused rating/comment states
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                // setLoading(true); // Initial state is true, no need to set
                setError(false);
    ...
    if (error || !product) return (
        <div className="min-h-screen bg-clinical-canvas flex flex-col items-center justify-center text-center p-6">
            ...
            <Link to="/shop" className="px-8 py-3 bg-clinical-ink text-white font-mono text-xs uppercase tracking-widest rounded-full hover:bg-black transition-all">
                Return to Shop
            </Link>
        </div>
    );
    ...
    return (
        <div className="bg-clinical-canvas min-h-screen pt-24 pb-20">
            <Helmet>
                <title>{`${product.name} | Olive Edge`}</title>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex items-center border border-clinical-ink/30 rounded-3xl overflow-hidden">
                                            ...
                                            <button onClick={() => setQty(Math.min(product.countInStock, qty + 1))} className="p-3 hover:bg-black/5"><FaPlus size={10} /></button>
                                        </div>
                                    </div>
            ...
            {/* 3. COMPLEMENTARY ASSETS (RELATED PRODUCTS) */}
