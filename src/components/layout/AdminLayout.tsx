        ...
        <div className="flex min-h-screen bg-clinical-canvas">
            ...
            `}>
                ...
                <nav className="flex-1 p-6 space-y-2 overflow-y-auto">
                    <Link href="/admin/dashboard" className={navItemClass('/admin/dashboard')} onClick={() => setIsSidebarOpen(false)}>
                        <FaChartLine />
                        <span className="font-sans text-xs uppercase tracking-widest font-bold">Dashboard</span>
                    </Link>
                    <Link href="/admin/orders" className={navItemClass('/admin/orders')} onClick={() => setIsSidebarOpen(false)}>
                        <FaClipboardList />
                        <span className="font-sans text-xs uppercase tracking-widest font-bold">Orders</span>
                    </Link>
                    <Link href="/admin/products" className={navItemClass('/admin/product')} onClick={() => setIsSidebarOpen(false)}>
                        <FaBox />
                        <span className="font-sans text-xs uppercase tracking-widest font-bold">Products</span>
                    </Link>
                    <Link href="/admin/users" className={navItemClass('/admin/users')} onClick={() => setIsSidebarOpen(false)}>
                        <FaUsers />
